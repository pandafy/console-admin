...
class addFacultyClass(wx.Dialog):
	...
	def basicGUI(self):
		...
		addFacultyBtn = wx.Button(self.panel,-1,'ADD')
		self.Bind(wx.EVT_BUTTON,self.adFaculty,addFacultyBtn)
		...
		sizer.Add(addFacultyBtn,pos = (10,12), flag = wx.ALIGN_CENTER, border = 5)
	...
	def chooseImage(self,e):
		...
		img = wx.Image(fileDialog.GetPath(), wx.BITMAP_TYPE_ANY)
		W = img.GetWidth()
		H = img.GetHeight()
		if W > H:
...
class editFacultyClass(wx.Dialog):
	def __init__(self,*args,**kwargs):
		super(editFacultyClass,self).__init__(*args,**kwargs)
		self.SetTitle("Edit")
		self.SetSize((750,600))
		self.PhotoMaxSize = 200		
		self.panel = wx.Panel(self)
	...
	def basicGUI(self):
		...
		SProfile = wx.StaticText(self.panel,-1,'Profile :')
		self.CProfile =wx.TextCtrl(self.panel,-1)
		img = wx.Image(self.FImage, wx.BITMAP_TYPE_ANY)
		self.imageCtrl = wx.StaticBitmap(self.panel, wx.ID_ANY,wx.BitmapFromImage(img))
